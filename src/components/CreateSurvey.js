...
const CreateSurvey = ({ handleSurveyAddition, surveys, onClose }) => {
  ...
  const handleAddQuestions = (id, text, type, choices, addBtn, addChoiceBtn) => {
    // console.log(id)
    ...
    if(addBtn.target.innerText === "Edit Question") {
      ...
      // console.log(document.querySelectorAll(`#${choiceParent} input`)[0]);
      // console.log(document.querySelectorAll(`#${choiceParent} input`)[1]);


      // document.querySelectorAll(`#${choiceParent} input`)[document.querySelectorAll(`#${choiceParent} input`).length - 1].disabled = false;
      // for(let i = 0; i < document.querySelectorAll(`#${choiceParent} input`).length; i++){

      //   document.querySelectorAll(`#${choiceParent} input`)[i].disabled = false;
      // }
      ...
      // addChoiceBtn.display.style.visibility = 'visible'; 
      addChoiceBtn.style.display = 'block'
    
      
      // console.log(document.querySelectorAll(`#${choiceParent} input`).length);
      ...
      // console.log(document.querySelectorAll(`#${choiceParent} input`)[0]);
      // console.log(document.querySelectorAll(`#${choiceParent} input`)[1]);
      // document.querySelectorAll(`#${choiceParent} input`)[document.querySelectorAll(`#${choiceParent} input`).length - 1].disabled = true;
      // for(let i = 0; i < document.querySelectorAll(`#${choiceParent} input`).length; i++){

      //   document.querySelectorAll(`#${choiceParent} input`)[i].disabled = true;
      // }
      ...
      // document.querySelectorAll(`#${choiceParent} input`).disabled = true;
    ...
    <div className=" w-full h-full flex flex-row flex-wrap justify-center items-center overflow-y-auto">
      <div className="cards border-brandBrown survey-basics w-72 h-85 m-2 flex flex-col">
        <form action="#" onSubmit={handleSubmit} className="flex flex-col">
          <label className="text-xl pl-4 pt-2 font-bold" htmlFor="surveyTitle">
            Survey Title
          </label>
          <textarea
            ...
            cols="70"
            rows="1"
            className="border-b-2 resize-none border-khaki w-60 mx-4 my-1 focus:outline-none text-sm px-1"
            type="text"
            id="surveyTitle"
            name="surveyTitle"
          />
          <label className="text-xl pl-4 pt-2 font-bold" htmlFor="surveyType">
            Survey Type
          </label>
          ...
          <div className=" m-1 flex flex-row justify-around items-center">
            ...
            <input
              ...
              className="focus:outline-none w-16 border-b-2 border-khaki"
              placeholder="minute"
              max={60}
              min={1}
              type="number"
          ...
          <div className="flex flex-col justify-start items-start">
            <label
              className="text-xl pl-3 pt-2 ml-1 font-bold"
            ...
            <textarea
              ...
              className=" focus:outline-none border-2 border-khaki m-4 mt-2 resize-none p-1"
              name=""
              id=""
              cols="30"
              rows="3"
            ...
            <div className="w-64 ml-4 flex justify-end items-center">
              <button
                ...
                type="submit"
                className="button bg-headerBlue text-white rounded-2xl h-6 w-20 flex justify-center items-center text-center text-md p-2 mb-2"
              >
                Submit
              </button>
            </div>
      ...
      <div className="cards survey-questions w-134  h-82 m-2  flex-col justify-evenly relative">
        ...
        <DragDropContext onDragEnd={handleOnDragEnd}>
            <Droppable droppableId="questions">
                {(provided) => (
        <div className="overflow-y-auto questions w-full h-72" {...provided.droppableProps} ref={provided.innerRef} >
            ...
            {questions &&
              questions.map((question, index) => (
                ...
                {...provided.draggableProps} {...provided.dragHandleProps} ref={provided.innerRef}
                  className="cards border-headerBlue border-l-8 border-t-2 question w-133 h-auto my-4 relative flex-col"
                >
                  <div
                  ...
                  <div className="font-bold text-brandBrown text-lg pl-1">
                    Question {questions.indexOf(question) + 1}
                  </div>
                  <div className="flex flex-row justify-evenly items-center">
                    <textarea
                      className="question-text focus:outline-none border-2 border-khaki m-2 mb-6 resize-none p-1 text-xs"
                      name=""
                      id=""
                      cols="70"
                      rows="2"
                    ...
                    <select
                      onChange={() =>
                        handleMultiple(
                          document.getElementsByClassName("questionType")[
                            question.id
                          ].value,
                          document.getElementsByClassName("show-Multiple")[
                            question.id
                          ]
                        )
                      }
                      className="questionType border-2 border-khaki text-md font-medium bg-khaki m-2 -mt-6 hover:cursor-pointer"
                      name="questionType"
                      id="questionType"
                    >
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        handleAddQuestions(
                          ...
                          e,
                          document.getElementById(`addChoice-${question.id}`)
                        );
                      }}
                      className="button bg-headerBlue absolute right-0 bottom-0 text-white rounded-2xl h-5 w-24 flex justify-center items-center text-center text-xs p-2 m-1"
                    >
                      Add Question
                    </button>
                  </div>
                          ...
                          question.choices.map((choice) => (
                            ...
                            >
                              ...
                              <div className="flex flex-row justify-center items-center">
                                <input
                                  type="text"
                                  className="focus:outline-none border-2 border-khaki m-2  p-1 text-xs h-7 w-96"
                                  name={question.id}
                                />
                                <svg
                                  onClick={() =>
